...
const PAGES = [
  { slug: "overview", label: "Overview", desc: "Library health, publishing trend and category mix." },
  { slug: "blogs", label: "Blogs", desc: "The full post library with SEO completeness per row." },
  { slug: "recommendations", label: "AI Recommendations", desc: "Prioritised fixes derived from the real library." },
  { slug: "engagement", label: "Engagement", desc: "Reach, reading time and what is actually performing." },
  { slug: "chat", label: "Chat with AI Agent", desc: "Ask anything — grounded in live platform data." },
];

export default function ContentAgentSpecialIndex() {
  return (
    <div style={{ minHeight: "100vh", background: "#fafbfe", padding: "48px 24px", fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif" }}>
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        ...
        <p style={{ fontSize: 14, color: "#4c5470", margin: "0 0 32px", lineHeight: 1.6 }}>
          The same content workspace, wired to each platform&apos;s own real blog library.
        </p>
            ...
            <div key={p.key} style={{ border: "1px solid #eef0f5", borderRadius: 16, background: "#fff", padding: 20, boxShadow: "0 2px 10px rgba(25,34,75,0.035)" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {PAGES.map((page) => (
                  <Link
                    key={page.slug}
                    href={`/content-agent-special/${p.key}/${page.slug}`}
