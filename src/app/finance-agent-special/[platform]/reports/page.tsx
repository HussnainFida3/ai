...
const DONUT_COLORS = ["#8b5cf6", "#f43f5e", "#f59e0b", "#38bdf8", "#94a3b8"];

function buildConicGradient(source: Array<{ value: number }>, total: number): string {
  if (total <= 0 || source.length === 0) return "conic-gradient(rgba(255,255,255,.08) 0 100%)";
  let acc = 0;
  ...
  return <div className="fm-app">
    <style>{`
      *{box-sizing:border-box}
      html,body,#root{margin:0;min-height:100%;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif;color:#f1f5f9;background:#030712}
      body{overflow-x:hidden}button,input{font:inherit}
      .fm-app{width:100%;min-height:1024px;background:#030712;display:flex}
      .sidebar{position:fixed;left:0;top:0;bottom:0;width:223px;background:linear-gradient(180deg,#0d1526 0%,#05080f 100%);color:#fff;z-index:20}
      .brand{height:83px;background:#0b1220;color:#f43f5e;display:flex;align-items:center;padding:12px 20px 9px;gap:8px}
      .brand img{width:38px;height:42px;object-fit:contain}
      .brand-name{font-size:22px;font-weight:800;line-height:23px;letter-spacing:-.8px;background:linear-gradient(90deg,#8b5cf6,#38bdf8);-webkit-background-clip:text;color:transparent}
      .brand-sub{font-size:10px;color:#f43f5e;text-align:center;margin-top:1px;letter-spacing:.05px}
      ...
      .nav-btn:hover{background:rgba(139,92,246,.18)}.nav-btn.active{background:linear-gradient(95deg,#7c3aed,#8b5cf6);box-shadow:inset 0 0 0 1px rgba(255,255,255,.04)}
      .finance-wrap{margin-top:0}.finance-head{height:38px;width:195px;border:0;border-radius:6px;background:linear-gradient(95deg,#7c3aed,#8b5cf6);color:#fff;display:flex;align-items:center;justify-content:space-between;padding:0 12px;cursor:pointer;font-size:14px}
      ...
      .sub-btn:hover{background:rgba(139,92,246,.2)}.sub-btn.active{background:linear-gradient(95deg,#7c3aed,#8b5cf6)}
      ...
      .top{height:84px;background:#0b1220;border-bottom:1px solid rgba(255,255,255,.07);display:flex;align-items:center;justify-content:space-between;padding:0 20px 0 25px}
      .ai-header{display:flex;align-items:center;gap:12px}.ai-head-icon{width:45px;height:45px;border-radius:50%;background:#0d1526;display:grid;place-items:center;color:#f1f5f9}
      .ai-heading{font-size:20px;font-weight:760;line-height:22px;letter-spacing:-.3px}.ai-heading .spark{color:#8b5cf6;margin-left:4px}.ai-sub{font-size:12px;color:#94a3b8;margin-top:5px}
      .top-right{display:flex;align-items:center;gap:17px}.top-date{width:220px;height:43px;border:1px solid rgba(255,255,255,.07);border-radius:8px;background:#0b1220;display:flex;align-items:center;justify-content:space-between;padding:0 14px;color:#f1f5f9;font-size:13px;cursor:pointer}
      .top-date span{display:flex;align-items:center;gap:12px}.notification{position:relative;width:43px;height:43px;border-radius:50%;background:#0b1220;border:1px solid rgba(255,255,255,.07);display:grid;place-items:center;color:#94a3b8;cursor:pointer}.badge{position:absolute;right:-1px;top:-2px;background:#f43f5e;color:#fff;font-size:9px;font-weight:700;width:16px;height:16px;border-radius:50%;display:grid;place-items:center;border:2px solid #0b1220}
      .profile{display:flex;align-items:center;gap:10px}.avatar{width:42px;height:42px;border-radius:50%;background:linear-gradient(145deg,#0d1526,#1a1433);position:relative;overflow:hidden}.avatar:before{content:"";position:absolute;left:14px;top:8px;width:15px;height:15px;border-radius:50%;background:#94a3b8}.avatar:after{content:"";position:absolute;left:8px;bottom:-1px;width:27px;height:19px;border-radius:16px 16px 5px 5px;background:#94a3b8}.profile-name{font-size:13px;font-weight:750}.profile-role{font-size:9px;color:#94a3b8;margin-top:3px}.profile .chev{margin-left:10px}
      ...
      .title-row{height:51px;display:flex;justify-content:space-between;align-items:flex-start}.title h1{font-size:21px;margin:0;font-weight:760;letter-spacing:-.4px}.title p{font-size:12px;color:#94a3b8;margin:5px 0 0}
      .actions{display:flex;gap:11px}.select{height:37px;min-width:111px;border:1px solid rgba(255,255,255,.07);background:#0b1220;border-radius:7px;padding:0 12px;display:flex;align-items:center;justify-content:space-between;gap:16px;color:#f1f5f9;font-size:11px;cursor:pointer}.export{height:37px;border:1px solid rgba(139,92,246,.4);color:#8b5cf6;background:#0b1220;border-radius:7px;padding:0 13px;display:flex;align-items:center;gap:7px;font-size:11px;font-weight:650;cursor:pointer}
      .metrics{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px;margin-bottom:11px}.metric{height:119px;background:#0b1220;border:1px solid rgba(255,255,255,.07);border-radius:8px;box-shadow:0 2px 8px rgba(0,0,0,.35);padding:16px 15px;position:relative}.metric-top{display:flex;gap:12px}.micon{width:40px;height:40px;border-radius:11px;display:grid;place-items:center;flex:0 0 auto}.m-purple{background:rgba(139,92,246,.16);color:#8b5cf6}.m-green{background:rgba(34,197,94,.16);color:#22c55e}.m-red{background:rgba(244,63,94,.16);color:#f43f5e}.m-blue{background:rgba(56,189,248,.16);color:#38bdf8}.m-label{font-size:11px;color:#f1f5f9;margin-top:1px}.m-value{font-size:18px;font-weight:780;letter-spacing:-.4px;margin-top:5px;white-space:nowrap}.m-growth{display:block;font-size:9.5px;color:#22c55e;margin-top:6px}.m-growth.red{color:#f43f5e}.m-sub{font-size:8.5px;color:#94a3b8;margin-top:5px}.cash .m-sub{margin-left:52px;margin-top:12px}
      .charts{display:grid;grid-template-columns:1.05fr 1fr;gap:9px;margin-bottom:11px}.chart-card{height:281px;background:#0b1220;border:1px solid rgba(255,255,255,.07);border-radius:8px;box-shadow:0 2px 8px rgba(0,0,0,.35);padding:16px 19px}.card-head{display:flex;justify-content:space-between;align-items:center}.card-title{font-size:12.5px;font-weight:750}.legend{display:flex;gap:20px;font-size:9px;color:#94a3b8;margin:11px 0 7px 34px}.legend i{width:8px;height:8px;border-radius:50%;display:inline-block;margin-right:6px}.purple-dot{background:#8b5cf6}.pink-dot{background:#f43f5e}
      .linechart{height:193px;position:relative;padding:8px 0 22px 40px}.grid{position:absolute;left:40px;right:0;top:8px;bottom:23px;display:flex;flex-direction:column;justify-content:space-between}.grid i{height:1px;background:rgba(255,255,255,.07)}.ylabels{position:absolute;left:0;top:5px;bottom:21px;display:flex;flex-direction:column;justify-content:space-between;font-size:9px;color:#94a3b8}.xlabels{position:absolute;left:40px;right:0;bottom:0;display:flex;justify-content:space-between;font-size:8.5px;color:#94a3b8}.linechart svg{position:absolute;left:40px;right:0;top:8px;width:calc(100% - 40px);height:158px}.revline{fill:none;stroke:#8b5cf6;stroke-width:2.2}.expline{fill:none;stroke:#f43f5e;stroke-width:2.2;stroke-dasharray:5 4}
      .profit-body{display:flex;align-items:center;height:210px;gap:25px}.donut{width:177px;height:177px;border-radius:50%;display:grid;place-items:center;flex:0 0 auto}.donut-hole{width:115px;height:115px;border-radius:50%;background:#0b1220;display:grid;place-items:center;align-content:center;text-align:center}.donut-hole span{font-size:11px;color:#94a3b8}.donut-hole strong{font-size:15px;margin-top:5px}.profit-list{display:flex;flex-direction:column;gap:15px;font-size:10px;overflow:auto;max-height:210px}.profit-row{display:grid;grid-template-columns:12px 1fr;gap:8px}.profit-row b{font-size:11px;font-weight:500}.profit-row small{display:block;font-size:9.5px;color:#94a3b8;margin-top:3px}.profit-row .dot{width:9px;height:9px;border-radius:50%;display:inline-block;margin-top:3px}.p1{background:#8b5cf6}.p2{background:#f43f5e}.p3{background:#f59e0b}.p4{background:#38bdf8}.p5{background:#94a3b8}
      .reports{min-height:200px;background:#0b1220;border:1px solid rgba(255,255,255,.07);border-radius:8px;box-shadow:0 2px 8px rgba(0,0,0,.35);padding:17px 15px}.reports-desc{font-size:10px;color:#94a3b8;margin:6px 0 12px}.table{width:100%;border-collapse:collapse;table-layout:fixed}.table th{height:31px;background:#0d1526;font-size:9px;font-weight:550;color:#f1f5f9;text-align:left;padding:0 7px}.table td{padding:10px 7px;border-bottom:1px solid rgba(255,255,255,.05);font-size:9px;color:#f1f5f9;vertical-align:top}.table th:nth-child(1){width:19%}.table th:nth-child(2){width:33%}.table th:nth-child(3){width:26%}.table th:nth-child(4){width:14%}.table th:nth-child(5){width:8%}.report-name{display:flex;align-items:center;gap:10px;font-weight:700;color:#f1f5f9}.report-icon{width:31px;height:31px;border-radius:8px;display:grid;place-items:center;flex:0 0 auto}.ri1{background:rgba(139,92,246,.16);color:#8b5cf6}.view{height:29px;border:1px solid rgba(139,92,246,.4);background:#0b1220;color:#8b5cf6;border-radius:6px;padding:0 9px;font-size:9px;display:inline-flex;align-items:center;gap:7px;cursor:pointer;white-space:nowrap}.view:disabled{opacity:.6;cursor:default}.dots{border:0;background:none;cursor:pointer;color:#94a3b8;margin-left:6px}
      .agent{background:#0b1220;border:1px solid rgba(255,255,255,.07);border-radius:8px;box-shadow:0 2px 8px rgba(0,0,0,.35);min-height:906px;padding:16px 10px}.agent-title{font-size:12.5px;font-weight:750;display:flex;justify-content:space-between}.online{font-size:10px;color:#22c55e;margin-top:6px}.online i{width:9px;height:9px;background:#22c55e;border-radius:50%;display:inline-block;margin-right:5px}.robot{height:132px;display:grid;place-items:center;position:relative}.robot-head{width:95px;height:86px;border-radius:45px 45px 34px 34px;background:linear-gradient(145deg,#0d1526,#1a1433);box-shadow:0 7px 15px rgba(139,92,246,.25);position:relative;border:2px solid rgba(139,92,246,.4)}.robot-head:before{content:"";position:absolute;left:21px;top:27px;width:53px;height:37px;border-radius:18px;background:#05080f;box-shadow:inset 0 0 15px rgba(139,92,246,.4)}.robot-head:after{content:"••";position:absolute;left:31px;top:29px;color:#8b5cf6;font-size:18px;letter-spacing:11px}.robot-tie{position:absolute;bottom:5px;left:45px;width:13px;height:34px;background:#7c3aed;clip-path:polygon(20% 0,80% 0,100% 100%,50% 76%,0 100%)}.robot-ear{position:absolute;width:13px;height:34px;border-radius:8px;background:#2a2145;top:33px}.ear-l{left:59px}.ear-r{right:59px}.agent-text{font-size:11px;line-height:19px;color:#f1f5f9}.agent-text strong{font-size:12px}.quick{border-top:1px solid rgba(255,255,255,.05);margin-top:14px;padding-top:12px}.quick-title{font-size:12px;font-weight:750;margin-bottom:8px}.quick button{height:32px;width:100%;border:1px solid rgba(139,92,246,.4);background:#0b1220;border-radius:6px;color:#8b5cf6;text-align:left;padding:0 10px;display:flex;align-items:center;gap:8px;font-size:9.5px;margin-bottom:6px;cursor:pointer}.insights{margin-top:14px;border-top:1px solid rgba(255,255,255,.05);padding-top:13px}.insights-title{font-size:12px;font-weight:750;margin-bottom:11px}.insight{display:flex;gap:9px;margin:10px 0;font-size:9.5px;line-height:15px}.insight-icon{width:28px;height:28px;border-radius:50%;display:grid;place-items:center;flex:0 0 auto}.ig{background:rgba(34,197,94,.16);color:#22c55e}.io{background:rgba(245,158,11,.16);color:#f59e0b}.ib{background:rgba(56,189,248,.16);color:#38bdf8}.ip{background:rgba(139,92,246,.16);color:#8b5cf6}.chat{margin-top:17px;height:48px;border:1px solid rgba(255,255,255,.07);border-radius:8px;display:flex;align-items:center;padding:0 7px 0 11px}.chat input{border:0;outline:0;flex:1;font-size:9.5px;color:#f1f5f9;background:transparent}.send{width:31px;height:31px;border-radius:50%;border:0;background:#8b5cf6;color:#fff;display:grid;place-items:center;cursor:pointer}.disclaimer{text-align:center;font-size:8px;color:#5b6780;line-height:14px;margin-top:20px}
      .date-menu{position:absolute;right:165px;top:62px;width:220px;background:#0d1526;border:1px solid rgba(255,255,255,.07);border-radius:8px;box-shadow:0 12px 30px rgba(0,0,0,.45);padding:6px;z-index:50}.date-menu button{width:100%;border:0;background:#0d1526;text-align:left;padding:9px;border-radius:5px;font-size:11px;cursor:pointer;color:#f1f5f9}.date-menu button:hover{background:rgba(139,92,246,.16);color:#8b5cf6}
      .toast{position:fixed;right:24px;bottom:22px;background:#0b1220;color:#fff;border-radius:8px;padding:11px 15px;font-size:11px;box-shadow:0 14px 35px rgba(0,0,0,.45);z-index:100;animation:in .22s ease-out}@keyframes in{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}
    ...
    <main className="main">
      ...
      <div className="page">
        <section className="main-col">
          ...
          <div className="charts">
            <div className="chart-card">
              ...
              <div className="linechart">
                ...
                {breakdown.loading || trend.loading ? (
                  <div style={{position:"absolute",left:40,right:0,top:8,height:158,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:"#94a3b8"}}>Loading real data…</div>
                ) : trend.error ? (
                  <div style={{position:"absolute",left:40,right:0,top:8,height:158,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:"#f43f5e"}}>Live data unavailable: {trend.error}</div>
                ) : histPts.length===0 ? (
                  <div style={{position:"absolute",left:40,right:0,top:8,height:158,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:"#94a3b8"}}>No data yet.</div>
                ) : (
                  <svg viewBox={`0 0 ${CHART_W} ${CHART_H}`} preserveAspectRatio="none">
                    <defs><linearGradient id="rfill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#8b5cf6" stopOpacity=".13"/><stop offset="1" stopColor="#8b5cf6" stopOpacity=".01"/></linearGradient></defs>
            ...
            <div className="chart-card">
              ...
              {breakdown.loading ? (
                <div style={{height:210,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:"#94a3b8"}}>Loading real data…</div>
              ) : breakdown.error ? (
                <div style={{height:210,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:"#f43f5e"}}>Live data unavailable: {breakdown.error}</div>
              ) : donutSource.length===0 ? (
                <div style={{height:210,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:"#94a3b8"}}>No data yet.</div>
              ) : (
          ...
          <div className="reports" style={showSnapshot ? {height:"auto"} : undefined}>
            ...
            {reportError && <p style={{color:"#f43f5e",fontSize:10,marginTop:10}}>Report generation failed: {reportError}</p>}
            {showSnapshot && reportSnapshot && (
              <div style={{marginTop:12,padding:12,background:"#0d1526",border:"1px solid rgba(255,255,255,.07)",borderRadius:8}}>
        ...
        <aside className="right-agent">
          <div className="agent">
            <div className="agent-title">Finance Manager AI Agent <span style={{color:"#5b6780"}}>⌁</span></div>
