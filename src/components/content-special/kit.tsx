...
/**
 * Content Agent Special — shared UI kit.
 *
 * The SEO and Finance special pages were each written as a single
 * self-contained file, which meant re-implementing the sidebar, topbar, icon
 * set and stylesheet every time. This kit factors that work out once so a new
 * "special" agent is a set of page bodies rather than a set of stylesheets.
 * Those two existing agents are deliberately left untouched — this kit is
 * additive, and every class name below is namespaced under `.cs-` so nothing
 * here can collide with `ag-*`, `dc-*`, or the unnamespaced globals the
 * earlier special pages declare.
 *
...
export function ScoreRing({ value, max = 100, size = 118, color = "#8b5cf6", label = "Score" }: { value: number; max?: number; size?: number; color?: string; label?: string }) {
  ...
  return (
    <div className="cs-donut" style={{ width: size, height: size, background: `conic-gradient(${color} 0 ${pct}%, rgba(255,255,255,.08) ${pct}% 100%)` }}>
      ...
      {rows.map((r, i) => (
        <div className="cs-bar-row" key={r.label}>
          ...
          <div className="cs-bar-track">
            <div className="cs-bar-fill" style={{ width: `${(r.value / max) * 100}%`, background: colored ? r.color ?? SERIES[i % SERIES.length] : "#8b5cf6" }} />
          </div>
        </div>
      ))}
    </div>
...
export const CONTENT_NAV: NavItem[] = [
  ...
  { label: "Blogs", icon: "posts", slug: "blogs" },
  { label: "AI Recommendations", icon: "sparkle", slug: "recommendations" },
  { label: "Engagement", icon: "pulse", slug: "engagement" },
      ...
      <aside className="cs-sidebar">
        <div className="cs-brand">
          ...
          <div>
            ...
            <div className="cs-brand-tag">Content Agent Special</div>
          </div>
        </div>
        ...
        <nav className="cs-nav">
          {CONTENT_NAV.map((item) => {
            const href = `/content-agent-special/${platform}/${item.slug}`;
            return (
        ...
        <div className="cs-assistant">
          <div className="cs-assistant-title">Content AI Agent</div>
          <Robot />
          <p>I draft, score and schedule every post — and tell you which ones are worth rewriting.</p>
          <Link href={`/content-agent-special/${platform}/chat`} className="cs-assistant-btn">Ask the agent</Link>
        </div>
      </aside>
      ...
      <main className="cs-main">
        <header className="cs-topbar">
          <div className="cs-topbar-title">
            <span className="cs-topbar-mark"><Icon name="posts" size={20} /></span>
            <div>
              <h1>{label} — {title}</h1>
              <p>{subtitle}</p>
            </div>
...
/* ── Stylesheet ─────────────────────────────────────────────────────── */
const CSS = `
.cs-app{--cs-purple:#8b5cf6;--cs-ink:#f1f5f9;--cs-ink-soft:#94a3b8;--cs-muted:#5b6780;--cs-line:rgba(255,255,255,.07);--cs-line-soft:rgba(255,255,255,.05);--cs-bg:#030712;--cs-green:#22c55e;--cs-red:#f43f5e;--cs-amber:#f59e0b;
...
/* Sidebar */
.cs-sidebar{position:fixed;left:0;top:0;bottom:0;width:232px;background:#05080f;border-right:1px solid var(--cs-line);padding:20px 14px;display:flex;flex-direction:column;z-index:10;overflow-y:auto}
...
.cs-back:hover{background:rgba(255,255,255,.06);color:var(--cs-ink)}
...
.cs-nav-item svg{color:var(--cs-muted);flex:0 0 auto}
.cs-nav-item:hover{background:rgba(255,255,255,.04);color:var(--cs-ink)}
.cs-nav-item.active{background:linear-gradient(90deg,#7c3aed,#8b5cf6);color:#fff;font-weight:670;box-shadow:0 6px 14px rgba(139,92,246,.35)}
.cs-nav-item.active svg{color:#fff}

.cs-assistant{margin-top:auto;border:1px solid rgba(139,92,246,.18);border-radius:12px;background:linear-gradient(180deg,rgba(139,92,246,.06),#0b1220);padding:15px 13px;text-align:center}
.cs-assistant-title{font-size:13px;font-weight:750;color:var(--cs-purple)}
.cs-assistant p{font-size:11px;line-height:18px;color:var(--cs-ink-soft);margin:8px 0 12px}
.cs-assistant-btn{display:block;height:35px;line-height:35px;border-radius:8px;background:linear-gradient(90deg,#7c3aed,#8b5cf6);color:#fff;font-size:12px;font-weight:650;box-shadow:0 5px 12px rgba(139,92,246,.35)}

/* Robot */
.cs-robot{width:96px;height:92px;position:relative;margin:10px auto 2px}
.cs-robot span{position:absolute}
.cs-robot-antenna{left:45px;top:0;width:6px;height:13px;background:#423868;border-radius:5px}
.cs-robot-antenna:after{content:"";position:absolute;width:9px;height:9px;border-radius:50%;background:#8b5cf6;left:-1.5px;top:-5px}
.cs-robot-head{left:22px;top:11px;width:53px;height:44px;border-radius:17px;background:linear-gradient(145deg,#262b45,#14172a);box-shadow:inset 0 -5px 9px rgba(0,0,0,.35),0 5px 8px rgba(0,0,0,.3)}
...
.cs-robot-face i{width:6px;height:6px;border-radius:50%;background:#a78bfa;box-shadow:0 0 7px #8b5cf6}
.cs-robot-ear{top:26px;width:11px;height:20px;border-radius:6px;background:#423868}
.cs-robot-ear.l{left:14px}.cs-robot-ear.r{right:14px}
.cs-robot-body{left:33px;top:52px;width:31px;height:33px;border-radius:11px 11px 14px 14px;background:linear-gradient(145deg,#262b45,#14172a);box-shadow:inset 0 -5px 8px rgba(0,0,0,.35)}
.cs-robot-arm{width:13px;height:28px;border-radius:9px;background:#423868;top:55px}
...
.cs-topbar{min-height:86px;display:flex;align-items:center;justify-content:space-between;gap:16px;padding:16px 24px;background:#0b1220;border-bottom:1px solid var(--cs-line);position:sticky;top:0;z-index:9;flex-wrap:wrap}
.cs-topbar-title{display:flex;align-items:center;gap:13px;min-width:0}
.cs-topbar-mark{width:44px;height:44px;border-radius:13px;background:rgba(139,92,246,.14);color:var(--cs-purple);display:grid;place-items:center;flex:0 0 auto}
...
.cs-icon-btn{width:38px;height:38px;border-radius:10px;border:1px solid var(--cs-line);background:#0b1220;color:var(--cs-ink-soft);display:grid;place-items:center}
.cs-icon-btn:hover{background:rgba(255,255,255,.06)}
.cs-user{display:flex;align-items:center;gap:9px}
.cs-avatar{width:36px;height:36px;border-radius:50%;background:linear-gradient(145deg,#8b5cf6,#4c1d95);color:#fff;display:grid;place-items:center;font-size:11px;font-weight:700}
.cs-user b{display:block;font-size:12px}
.cs-user small{display:block;font-size:10px;color:var(--cs-muted);margin-top:1px}
.cs-btn{height:36px;padding:0 14px;border-radius:8px;border:1px solid var(--cs-line);background:#0b1220;color:var(--cs-ink);font-size:12px;font-weight:600;display:inline-flex;align-items:center;gap:7px}
.cs-btn:hover{background:rgba(255,255,255,.06)}
.cs-btn-primary{border:0;background:linear-gradient(90deg,#7c3aed,#8b5cf6);color:#fff;box-shadow:0 5px 12px rgba(139,92,246,.35)}
.cs-content{padding:20px 24px 32px;display:flex;flex-direction:column;gap:14px}

/* Cards */
.cs-card{background:#0b1220;border:1px solid var(--cs-line);border-radius:12px}
...
.cs-stat{background:#0b1220;border:1px solid var(--cs-line);border-radius:12px;padding:16px 17px 13px;position:relative;overflow:hidden}
...
.cs-donut-hole{position:absolute;background:#0b1220;border-radius:50%;display:grid;place-items:center;align-content:center;text-align:center}
...
.cs-tip{position:absolute;top:0;transform:translateX(-50%);background:#161b36;border:1px solid rgba(255,255,255,.08);color:#fff;border-radius:8px;padding:8px 11px;font-size:10.5px;line-height:17px;pointer-events:none;box-shadow:0 12px 28px rgba(0,0,0,.5);white-space:nowrap;z-index:3}
...
.cs-bar-track{height:7px;border-radius:5px;background:rgba(255,255,255,.08);overflow:hidden}
...
.cs-table tbody tr:hover{background:rgba(255,255,255,.03)}
...
.cs-error{display:flex;gap:11px;align-items:flex-start;border:1px solid rgba(244,63,94,.25);background:rgba(244,63,94,.08);border-radius:11px;padding:13px 15px}
.cs-error-icon{width:28px;height:28px;border-radius:8px;background:rgba(244,63,94,.14);color:var(--cs-red);display:grid;place-items:center;flex:0 0 auto}
.cs-error b{display:block;font-size:12.5px;color:var(--cs-red)}
...
.cs-search{height:36px;display:flex;align-items:center;gap:8px;padding:0 12px;border:1px solid var(--cs-line);border-radius:8px;background:#0b1220;min-width:200px;color:var(--cs-muted)}
